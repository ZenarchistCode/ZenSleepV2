...
class ZenSleepVisualEffectHandlerClient extends ZenSleepEffectHandlerBase
{
	...
	{
		ZenSleepEffectsClientConfig cfg = GetZenSleepConfig().ClientEffectsConfig;
		if (!cfg)
			return;
		...
		bool allowHallucinate = (redFlashing && cfg.AllowHallucinationEffectRed) || (red && cfg.AllowHallucinationEffectRedFlashing);
		...
		{
			if (m_Player.GetZenSleepManager().IsSleeping() && cfg.AllowClosedEyesEffectSleeping)
			{
				sleepFX = 1.0;
			}
			...
			if (redFlashing)
			{
				...
				if (cfg.AmountToBlurWhenRedFlashing > 0)
				{
					blurFX = (cfg.AmountToBlurWhenRedFlashing / 100) * percentTimeSpentRedFlashing;
				}

				if (!m_Player.GetZenSleepManager().IsSleeping() && cfg.AmountToVignetteWhenRedFlashing > 0)
				{
					if (!m_Player.GetZenSleepManager().IsSleeping())
					{
						sleepFX = (cfg.AmountToVignetteWhenRedFlashing / 100) * percentTimeSpentRedFlashing;
						sleepFX_Speed = 0.4;
					}
				}
			}
			else 
			{
				...
				if (red)
				{
					...
					if (!m_Player.GetZenSleepManager().IsSleeping() && cfg.AmountToVignetteWhenRed > 0)
					{
						sleepFX = (cfg.AmountToVignetteWhenRed / 100) * percentTimeSpentRed;
						sleepFX_Speed = 0.25;
					}
				
					if (cfg.AmountToBlurWhenRed > 0)
						blurFX = (cfg.AmountToBlurWhenRed / 100) * percentTimeSpentRed;
				}
		...
		if (cfg.AllowClosedEyesEffectSleeping || m_IsGoingUncon)
		{
			if (m_SleepingVFX)
			{
		...
		if (cfg.AmountToBlurWhenRed > 0 || cfg.AmountToBlurWhenRedFlashing > 0)
		{
			if (m_BlurVFX)
			{
