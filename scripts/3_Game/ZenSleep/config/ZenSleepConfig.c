ref ZenSleepConfig g_ZenSleepConfig;

static ZenSleepConfig GetZenSleepConfig()
{
	if (!g_ZenSleepConfig) GetZenConfigRegister().RegisterConfig(ZenSleepConfig);
	return g_ZenSleepConfig;
}
...
modded class ZenConfigRegister
{
	override void RegisterPreload()
	{
		...
		RegisterType(ZenSleepConfig);
	}
}

class ZenSleepConfig_SyncPayload
{
	ref ZenSleepEffectsClientConfig ClientEffectsConfig;
	bool DebugMode;
}

class ZenSleepConfig : ZenConfigBase
{
	// -------------------------
	// CONFIG SETTINGS
	// -------------------------
	...
	{
		return JsonFileLoader<ZenSleepConfig>.LoadFile(path, this, err);
	}

	override bool WriteJson(string path, out string err)
	{
		return JsonFileLoader<ZenSleepConfig>.SaveFile(path, this, err);
	}
	
	override bool BuildSyncPayload(out string payload, out string err)
	{
		ZenSleepConfig_SyncPayload snap = new ZenSleepConfig_SyncPayload();
		snap.ClientEffectsConfig = ClientEffectsConfig;
		snap.DebugMode = DebugMode;
		
		return JsonFileLoader<ZenSleepConfig_SyncPayload>.MakeData(snap, payload, err, false);
	}

	override bool ApplySyncPayload(string payload, out string err)
	{
		ZenSleepConfig_SyncPayload snap = new ZenSleepConfig_SyncPayload();
		if (!JsonFileLoader<ZenSleepConfig_SyncPayload>.LoadData(payload, snap, err))
		{
			return false;
		}

		ClientEffectsConfig = snap.ClientEffectsConfig;
