modded class PlayerBase
{
	...
	void SetZenSleepFxID(int id)
	{
		...
		if (id != -1)
		{
			g_Game.GetCallQueue(CALL_CATEGORY_SYSTEM).Remove(ResetZenSleepFxID);	// in case one is queued already
			g_Game.GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(ResetZenSleepFxID, 100);
		}
	}
	...
	override void CF_OnStoreSave(CF_ModStorageMap storage)
	{
		...
		auto ctx = storage["ZenSleep"];
		if (!ctx) return;
		
		float pauseDrainEffectSecsZS = 0;
		float inabilityToSleepEffectSecsZS = 0;
		
		if (GetZenSleepManager())
		{
			pauseDrainEffectSecsZS = GetZenSleepManager().GetFatigueDrainPauseSeconds();
			inabilityToSleepEffectSecsZS = GetZenSleepManager().GetInabilityToSleepEffectSecs();
		}

		ctx.Write(pauseDrainEffectSecsZS);
		ctx.Write(inabilityToSleepEffectSecsZS);
	}
	...
	override bool CF_OnStoreLoad(CF_ModStorageMap storage)
	{
		...
		if (ctx.GetVersion() >= 1)
		{
			...
			if (!ctx.Read(pauseDrainEffectSecsZS))
				return false;

			if (!ctx.Read(inabilityToSleepEffectSecsZS))
				return false;

			if (GetZenSleepManager())
				GetZenSleepManager().SetPersistenceVariables(pauseDrainEffectSecsZS, inabilityToSleepEffectSecsZS);
